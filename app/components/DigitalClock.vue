	...
	onMounted(() => {
		...
		const addDigits = (number: number) => {
			const initGroup = (num: number, padding = 2) => {
				...
				const digits = [...`${currentNum}`.padStart(padding, "0")].map(
					(digit) => {
...
<style scoped>
	...
	:deep(
		.digits .group .digit[data-digit="2"] :not(.top.left, .bottom.right)
	) {
		--act: 1;
	}
	...
	:deep(
		.digits .group .digit[data-digit="5"] :not(.top.right, .bottom.left)
	) {
		--act: 1;
	}
	...
	:deep(.shadow.shadow1) {
		opacity: 0.5;
		filter: drop-shadow(0px 0px 4px currentColor)
			drop-shadow(0px 0px 10px currentColor) blur(4em);
	}
