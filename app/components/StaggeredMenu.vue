<template>
	<div class="sm-container" :style="{ '--sm-accent': accentColor }">
		<!-- Hamburger / Open toggle (only interactive when menu is closed) -->
		<button
			ref="toggleBtnRef"
			type="button"
			class="sm-toggle transition-opacity duration-300"
		...
		<Teleport to="body">
			...
			>
				<!-- Dedicated Close Button on top of the overlay -->
				<button
					v-if="isOpen"
					type="button"
					class="sm-close-btn"
					...
					@click="closeMenu"
				>
					<span class="sm-toggle-text">Close</span>
					<span class="sm-icon" aria-hidden="true">
						<span class="sm-icon-bar sm-icon-bar--close-1" />
						<span class="sm-icon-bar sm-icon-bar--close-2" />
					</span>
				</button>
...
<script setup lang="ts">
	...
	function setupGsap() {
		...
		if (preLayersRef.value) {
			preLayerEls = Array.from(
				preLayersRef.value.querySelectorAll(".sm-prelayer"),
			) as HTMLElement[];
		}
		...
		if (preLayersRef.value)
			gsap.set(preLayersRef.value, { xPercent: 0, opacity: 1 });
	...
	function buildOpenTimeline(): gsap.core.Timeline | null {
		...
		const labelEls = Array.from(
			panel.querySelectorAll(".sm-nav-label"),
		) as HTMLElement[];
		const numberEls = Array.from(
			panel.querySelectorAll(".sm-nav-list[data-numbering] .sm-nav-item"),
		) as HTMLElement[];
		...
		// Main panel follows
		const panelStart =
			(layers.length > 0 ? (layers.length - 1) * 0.07 : 0) +
			(layers.length ? 0.08 : 0);
			...
			onComplete: () => {
				...
				const numbers = Array.from(
					panel.querySelectorAll(
						".sm-nav-list[data-numbering] .sm-nav-item",
					),
				) as HTMLElement[];
				const sTitle = panel.querySelector(
					".sm-socials-title",
				) as HTMLElement | null;
