...
const mockPosts = [
  {
    ...
    excerpt: 'Discover best practices and architecture patterns for building high-performance Vue and Nuxt applications at scale.',
    category: 'Frontend',
    date: 'May 20, 2026',
    readTime: '5 min read',
...
describe('BlogSection', () => {
    it('renders heading and descriptive text', async () => {
        ...
        expect(wrapper.text()).toContain('Sharing my thoughts, design patterns, and engineering experiences');
    });

    it('renders mocked blog posts', async () => {
        const wrapper = await mountSuspended(BlogSection);
        
        expect(wrapper.text()).toContain('Building Scalable Nuxt Applications in 2026');
        expect(wrapper.text()).toContain('Discover best practices and architecture patterns');
